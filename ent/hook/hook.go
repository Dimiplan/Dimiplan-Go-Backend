...
// function as Planner mutator.
type PlannerFunc func(context.Context, *ent.PlannerMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PlannerFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PlannerMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PlannerMutation", m)
}
