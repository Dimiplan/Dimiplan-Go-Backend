...
func (_c *TaskCreate) SetPlannerID(id int) *TaskCreate {
	_c.mutation.SetPlannerID(id)
	return _c
}

// SetPlanner sets the "planner" edge to the Planner entity.
func (_c *TaskCreate) SetPlanner(v *Planner) *TaskCreate {
	return _c.SetPlannerID(v.ID)
}
...
func (_c *TaskCreate) check() error {
	...
	if len(_c.mutation.PlannerIDs()) == 0 {
		return &ValidationError{Name: "planner", err: errors.New(`ent: missing required edge "Task.planner"`)}
	}
	return nil
}
...
func (_c *TaskCreate) createSpec() (*Task, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.PlannerIDs(); len(nodes) > 0 {
