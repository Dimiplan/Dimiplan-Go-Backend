...
func (_q *TaskQuery) QueryPlanner() *PlannerQuery {
...
func (_q *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withPlanner: _q.withPlanner.Clone(),
...
func (_q *TaskQuery) WithPlanner(opts ...func(*PlannerQuery)) *TaskQuery {
	query := (&PlannerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPlanner = query
	return _q
}
...
func (_q *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := _q.withPlanner; query != nil {
		if err := _q.loadPlanner(ctx, query, nodes, nil,
			func(n *Task, e *Planner) { n.Edges.Planner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TaskQuery) loadPlanner(ctx context.Context, query *PlannerQuery, nodes []*Task, init func(*Task), assign func(*Task, *Planner)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		if nodes[i].planner_tasks == nil {
			continue
		}
		fk := *nodes[i].planner_tasks
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "planner_tasks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
