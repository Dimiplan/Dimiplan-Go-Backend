...
func (_u *TaskUpdate) SetPlannerID(id int) *TaskUpdate {
	_u.mutation.SetPlannerID(id)
	return _u
}

// SetPlanner sets the "planner" edge to the Planner entity.
func (_u *TaskUpdate) SetPlanner(v *Planner) *TaskUpdate {
	return _u.SetPlannerID(v.ID)
}
...
func (_u *TaskUpdate) ClearPlanner() *TaskUpdate {
	_u.mutation.ClearPlanner()
	return _u
}
...
func (_u *TaskUpdate) check() error {
	...
	if _u.mutation.PlannerCleared() && len(_u.mutation.PlannerIDs()) > 0 {
		return errors.New(`ent: clearing a required unique edge "Task.planner"`)
	}
	return nil
}
...
func (_u *TaskUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	...
	if _u.mutation.PlannerCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   task.PlannerTable,
			Columns: []string{task.PlannerColumn},
			Bidi:    false,
	...
	if nodes := _u.mutation.PlannerIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   task.PlannerTable,
...
// SetPlannerID sets the "planner" edge to the Planner entity by ID.
func (_u *TaskUpdateOne) SetPlannerID(id int) *TaskUpdateOne {
	_u.mutation.SetPlannerID(id)
	return _u
}

// SetPlanner sets the "planner" edge to the Planner entity.
func (_u *TaskUpdateOne) SetPlanner(v *Planner) *TaskUpdateOne {
	return _u.SetPlannerID(v.ID)
}
...
// ClearPlanner clears the "planner" edge to the Planner entity.
func (_u *TaskUpdateOne) ClearPlanner() *TaskUpdateOne {
	_u.mutation.ClearPlanner()
	return _u
}
