...
type UserQuery struct {
	...
	withPlanners  *PlannerQuery
	withChatrooms *ChatRoomQuery
...
// QueryPlanners chains the current query on the "planners" edge.
func (_q *UserQuery) QueryPlanners() *PlannerQuery {
	query := (&PlannerClient{config: _q.config}).Query()
...
// QueryChatrooms chains the current query on the "chatrooms" edge.
func (_q *UserQuery) QueryChatrooms() *ChatRoomQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withPlanners:  _q.withPlanners.Clone(),
		withChatrooms: _q.withChatrooms.Clone(),
...
// WithPlanners tells the query-builder to eager-load the nodes that are connected to
// the "planners" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithPlanners(opts ...func(*PlannerQuery)) *UserQuery {
	query := (&PlannerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPlanners = query
	return _q
}

// WithChatrooms tells the query-builder to eager-load the nodes that are connected to
// the "chatrooms" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithChatrooms(opts ...func(*ChatRoomQuery)) *UserQuery {
	query := (&ChatRoomClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withChatrooms = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withPlanners; query != nil {
		if err := _q.loadPlanners(ctx, query, nodes,
			func(n *User) { n.Edges.Planners = []*Planner{} },
			func(n *User, e *Planner) { n.Edges.Planners = append(n.Edges.Planners, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withChatrooms; query != nil {
		if err := _q.loadChatrooms(ctx, query, nodes,
			func(n *User) { n.Edges.Chatrooms = []*ChatRoom{} },
			func(n *User, e *ChatRoom) { n.Edges.Chatrooms = append(n.Edges.Chatrooms, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadPlanners(ctx context.Context, query *PlannerQuery, nodes []*User, init func(*User), assign func(*User, *Planner)) error {
	...
	query.withFKs = true
	query.Where(predicate.Planner(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.PlannersColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_planners
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_planners" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_planners" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserQuery) loadChatrooms(ctx context.Context, query *ChatRoomQuery, nodes []*User, init func(*User), assign func(*User, *ChatRoom)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.user_chatrooms
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_chatrooms" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_chatrooms" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
